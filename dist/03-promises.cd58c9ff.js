...
form.addEventListener("submit", (e)=>{
    e.preventDefault();
    const delay1 = parseInt(form.elements.delay.value);
    const step = parseInt(form.elements.step.value);
    const amount = parseInt(form.elements.amount.value);
    for(let i = 0; i < amount; i += 1)createPromise(i + 1, delay1 + i * step).then(({ position , delay  })=>{
        _notiflix.Notify.success(`✅ Fulfilled promise ${position} in ${delay}ms`);
    }).catch(({ position , delay  })=>{
        _notiflix.Notify.failure(`❌ Rejected promise ${position} in ${delay}ms`);
    });
});
